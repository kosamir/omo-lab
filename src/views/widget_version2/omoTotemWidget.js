...
function generateOmoStyle() {
  ...
  let style =
    values.bgColor !== config.DEFAULT_BACKGROUND
...
const show = (text, configurations) => {
  ...
  let powerToggler = widgetOnOffToggler(
    document.getElementById('ignite'),
    ['omo-totem-off'],
    ['omo-totem-on', 'omo-eye-on'],
    document.getElementById('applyOverides'),
  ); //collorToogler(document.getElementById('ignite'), 'omo-totem-on', 'omo-totem-off');
  document
    .getElementById('ignite')
    .addEventListener('click', powerToggler.toogle);
  /**TOTEM TOOGLER */
  var mainToogler = tooglerMain(
  ...
  document
    ...
    .addEventListener('click', mainToogler.toogle);
  /** FONT SIZE TOOGLER */
  var fontSizeToogler = tooglerMain(
    document.getElementById('font'),
  ...
  document
    .getElementById('font')
    .addEventListener('click', fontSizeToogler.toogle);
  /** MAIN FONT TYPE TOOGLER */
  var fontTypeToogler = tooglerMain(
    document.getElementById('font-type'),
  ...
  document
    .getElementById('font-type')
    .addEventListener('click', fontTypeToogler.toogle);

  var toogleFonts = pictureToogler(
    document.getElementById('font-type-container'),
    FONT_CHOOSER,
    document.getElementById('totem_body_ff'),
  );
  document
    .getElementById('font-type-up')
    .addEventListener('click', toogleFonts.up);
  document
    .getElementById('font-type-down')
    .addEventListener('click', toogleFonts.down);

  var fontWeightoogler = tooglerMain(
    document.getElementById('font-weight'),
  ...
  document
    ...
    .addEventListener('click', fontWeightoogler.toogle);
  document.getElementById('totem_font_weight').value = config.BODY_FONT_WEIGHT;
  var toogleWeight = pictureToogler(
    document.getElementById('font-weight-container'),
    FONT_WEIGHT_CHOOSER,
    document.getElementById('totem_font_weight'),
  );

  document
    .getElementById('font-weight-up')
    .addEventListener('click', toogleWeight.up);
  document
    .getElementById('font-weight-down')
    .addEventListener('click', toogleWeight.down);

  var fontSpacingToogler = tooglerMain(
    document.getElementById('font-spacing'),
  ...
  document
    .getElementById('font-spacing')
    .addEventListener('click', fontSpacingToogler.toogle);

  var toogleFontSpacing = pictureToogler(
    document.getElementById('font-spacing-container'),
    FONT_SPACING,
    document.getElementById('totem_bspacing'),
  );
  document
    .getElementById('font-spacing-up')
    .addEventListener('click', toogleFontSpacing.up);
  document
    .getElementById('font-spacing-down')
    .addEventListener('click', toogleFontSpacing.down);

  var lineSpacingToogler = tooglerMain(
    document.getElementById('line-spacing'),
  ...
  document
    .getElementById('line-spacing')
    .addEventListener('click', lineSpacingToogler.toogle);

  var backgroundToogler = tooglerMain(
    document.getElementById('background'),
  ...
  document
    ...
    .addEventListener('click', backgroundToogler.toogle);

  let whiteToggler = collorToogler(
    document.getElementById('EFF3EE'),
    'omo-totem-background-white-on',
    'omo-totem-background-white',
  );
  document
    .getElementById('EFF3EE')
    .addEventListener('click', whiteToggler.toogle);

  let greyToggler = collorToogler(
    document.getElementById('D9D8D8'),
    'omo-totem-background-grey-on',
    'omo-totem-background-grey',
  );
  document
    .getElementById('D9D8D8')
    .addEventListener('click', greyToggler.toogle);

  let blackToggler = collorToogler(
    document.getElementById('231F20'),
    'omo-totem-background-black-on',
    'omo-totem-background-black',
  );
  document
    .getElementById('231F20')
    .addEventListener('click', blackToggler.toogle);

  let blueToggler = collorToogler(
    document.getElementById('9FB5DE'),
    'omo-totem-background-blue-on',
    'omo-totem-background-blue',
  );
  document
    .getElementById('9FB5DE')
    .addEventListener('click', blueToggler.toogle);

  let pinkToggler = collorToogler(
    document.getElementById('B894C4'),
    'omo-totem-background-pink-on',
    'omo-totem-background-pink',
  );
  document
    .getElementById('B894C4')
    .addEventListener('click', pinkToggler.toogle);

  let orangeToggler = collorToogler(
    document.getElementById('E2B0AF'),
    'omo-totem-background-orange-on',
    'omo-totem-background-orange',
  );
  document
    .getElementById('E2B0AF')
    .addEventListener('click', orangeToggler.toogle);
  ...
  config
    ...
    .then(message => {
      ...
      var toogleSize = sizeToogler(
        parseInt(config.BODY_FONT_SIZE),
        parseInt(config.BODY_FONT_SIZE_MAX),
        1,
        document.getElementById('totem_bsize'),
      );
      document
        .getElementById('font-size-up')
        .addEventListener('click', toogleSize.up); //applyOmoStylesTotem
      document
        .getElementById('font-size-down')
        .addEventListener('click', toogleSize.down); //applyOmoStylesTotem
      /** LINE SPACING UP/DOWN TOOGLER */
      var lineSpacingToogler = sizeToogler(
        parseInt(config.BODY_LINE_HEIGHT),
        parseInt(config.BODY_LINE_HEIGHT_MAX),
        0.05,
        document.getElementById('totem_bheight'),
      );
      document
        .getElementById('line-spacing-up')
        .addEventListener('click', lineSpacingToogler.up); //applyOmoStylesTotem
      document
        .getElementById('line-spacing-down')
        .addEventListener('click', lineSpacingToogler.down); //applyOmoStylesTotem
      /** SOME DEFAULT VALUES */
      DEFAULT_TOTEM_VALUES.push({
        id: 'totem_body_ff',
        value: config.BODY_FONT_FAMILY,
      });
      DEFAULT_TOTEM_VALUES.push({
        id: 'totem_bsize',
      ...
      DEFAULT_TOTEM_VALUES.push({
        ...
        value: config.BODY_FONT_WEIGHT,
      });
      DEFAULT_TOTEM_VALUES.push({
        id: 'totem_bspacing',
        value: config.BODY_FONT_SPACING,
      });
      DEFAULT_TOTEM_VALUES.push({
        id: 'totem_bheight',
        value: config.BODY_LINE_HEIGHT,
      });
      DEFAULT_TOTEM_VALUES.push({
...
const COLOR_MAP = [
  {
    id: 'EFF3EE',
    on: false,
    css_open: 'omo-totem-background-white-on',
    css_close: 'omo-totem-background-white',
  },
  {
    id: 'D9D8D8',
    on: false,
    css_open: 'omo-totem-background-grey-on',
    css_close: 'omo-totem-background-grey',
  },
  {
    id: '231F20',
    on: false,
    css_open: 'omo-totem-background-black-on',
    css_close: 'omo-totem-background-black',
  },
  {
    id: '9FB5DE',
    on: false,
    css_open: 'omo-totem-background-blue-on',
    css_close: 'omo-totem-background-blue',
  },
  {
    id: 'B894C4',
    on: false,
    css_open: 'omo-totem-background-pink-on',
    css_close: 'omo-totem-background-pink',
  },
  {
    id: 'E2B0AF',
    on: false,
    css_open: 'omo-totem-background-orange-on',
    css_close: 'omo-totem-background-orange',
  },
];
...
const sizeToogler = (_start, _end, _step, _el) => {
  ...
  return {
    up: () => {
      ...
      element.value =
        current % 1 === 0 ? current : parseFloat(current).toFixed(2);
      applyOverides();
    },
    down: () => {
...
const pictureToogler = (el, choose, _hidden) => {
  ...
  return {
    ...
    down: () => {
      num_of_fonts =
        num_of_fonts <= 0 ? chooser.length - 1 : (num_of_fonts -= 1);
      let value = chooser[Math.abs(num_of_fonts) % chooser.length];
      element.setAttribute('class', value.style);
      hidden.value = value.value;
      applyOverides();
  ...
  return {
    toogle() {
      if (is_open) {
        ...
        parent.setAttribute(
          'class',
          typeof toogler != 'undefined' && toogler.isOpen()
            ? parent_open_style
            : parent_close_style,
        );
        if (child && child.length > 0)
          child.forEach(el => el.setAttribute('style', 'display:none'));
        is_open = false;
      } else {
        TOTEM_ELEMENTS_MAP.forEach(el => {
          if (el.id !== parent.id) {
            ...
            if (main.getAttribute('class') === el.choose) {
              ...
              let def = DEFAULT_TOTEM_VALUES.filter(e => el.hidden === e.id)[0]
                .value;
        ...
        if (child && child.length > 0)
          child.forEach(el => el.setAttribute('style', 'display:block'));
        is_open = true;
      }
    },
  };
